...
const MILESTONE = 100; // founding-circle goal — the real count grows toward it
...
export function Lab() {
  ...
  // REAL numbers only: the live waitlist count from the server (same source
  // the Waitlist section uses). No fabricated stats — honest social proof.
  ...
  useEffect(() => {
    ...
    fetch(`${WAITLIST_API_URL}/waitlist/count`)
      ...
      .catch(() => {
        /* server not up in dev — counter stays hidden */
      });
    return () => {
      alive = false;
    };
  ...
  // Only count up once the section scrolls into view, so the sweep is
  // actually seen (the fetch may resolve while the section is off-screen).
  useEffect(() => {
  ...
  // Count-up the real number when it first becomes visible — honest, alive.
  useEffect(() => {
    if (waitlistCount === null || !countVisible) return;
    if (reduced) {
    ...
    const tick = (t: number) => {
      if (cancelled) return;
      // clamp p to [0,1] — the first rAF timestamp can precede the captured
      // start, which would otherwise overshoot to count+1 for a frame
      const p = Math.min(1, Math.max(0, (t - start) / dur));
      const eased = 1 - Math.pow(1 - p, 3);
      setDisplayCount(Math.round(waitlistCount * eased));
  ...
  }, [waitlistCount, countVisible, reduced]);
  ...
  // counter pop when someone new joins the circle (visual only — the
  // waitlist number is real and never faked)
  useEffect(() => {
      ...
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center overflow-hidden">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at center, rgba(212,175,55,0.26) 0%, rgba(212,175,55,0.07) 45%, transparent 72%)",
            }}
          />
              ...
              {Array.from({ length: MAX_MEMBERS }, (_, i) => (
                ...
                >
                  ...
                  >
                    ...
                    <g transform="translate(-10 -10)" color="#d4af37" opacity="0.85">
                      <VibeIcon i={i} />
                    </g>
        ...
        <div className="pointer-events-none relative z-10 flex h-full w-full flex-col items-center justify-between px-6 py-12 sm:py-14">
          ...
          <div className="flex flex-col items-center gap-3">
            {/* real founding-circle progress — live server count toward the
                goal. The number is never fabricated; the milestone makes even
                a small count feel intentional and exclusive. */}
            {waitlistCount !== null && countVisible && (
              <>
                <div className="flex items-center gap-4">
                  <span className="flex flex-col items-center gap-0.5">
                    ...
                    <span className="u-label text-[11px] text-white/55">
                      founding members in
                    </span>
                  </span>
                  <span className="h-9 w-px bg-white/10" />
                  <span className="flex flex-col items-center gap-0.5">
                    <b className="font-mono text-2xl tabular-nums text-white/90">
                      {fmt(MILESTONE)}
                    </b>
                    <span className="u-label text-[11px] text-white/55">
                      circle goal
                ...
                <div className="h-1 w-48 overflow-hidden rounded-full bg-white/10">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-amber via-rose to-white/70 transition-[width] duration-700 ease-out"
                    style={{
                      // driven by displayCount so the bar sweeps in with the
                      // count-up — one animation, zero fabricated numbers
                      width: `${Math.min(
                        100,
                        Math.round((displayCount / MILESTONE) * 100),
                      )}%`,
                    }}
                  />
                </div>
